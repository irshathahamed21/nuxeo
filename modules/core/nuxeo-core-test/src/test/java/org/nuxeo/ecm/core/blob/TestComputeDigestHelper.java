...
public class TestComputeDigestHelper {
    ...
    public void testReplaceDigest() {
        ...
        assertEquals("newdigest", blob.getDigest());
    }
    ...
    public void testCoreSessionReplaceBlobDigest() {
        ...
        try (CapturingEventListener listener = new CapturingEventListener(BLOB_DIGEST_UPDATED)) {
            ...
            String oldDigest = session.replaceBlobDigest(doc1.getRef(), FOO_MD5, BAR_MD5, "newdigest");
            ...
            assertEquals("newdigest", properties.get(BLOB_DIGEST_UPDATED_NEW_DIGEST));
        }
        session.save();
        doc1.refresh();
