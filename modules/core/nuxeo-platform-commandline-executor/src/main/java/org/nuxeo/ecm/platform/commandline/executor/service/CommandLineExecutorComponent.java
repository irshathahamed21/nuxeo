...
public class CommandLineExecutorComponent extends DefaultComponent implements CommandLineExecutorService {
    ...
    public void start(ComponentContext context) {
        ...
        this.<CommandTesterDescriptor> getRegistryContributions(EP_CMDTESTER).forEach(desc -> {
            ...
            try {
                tester = desc.getTesterClass().getDeclaredConstructor().newInstance();
    ...
    protected EnvironmentDescriptor getEnvironmentDescriptor(String name) {
        return this.<EnvironmentDescriptor> getRegistryContribution(EP_ENV, name).orElse(null);
    }
    ...
    public ExecResult execCommand(String commandName, CmdParameters params) throws CommandNotAvailable {
        ...
        CommandLineDescriptor cmdDesc = this.<CommandLineDescriptor> getRegistryContribution(EP_CMD, commandName)
                                            // should not happen
                                            .orElseThrow(() -> new RuntimeException(
                                                    "Command " + commandName + " is not be available"));
        EnvironmentDescriptor globalEnv = getEnvironmentDescriptor("");
        EnvironmentDescriptor commandEnv = getEnvironmentDescriptor(commandName);
        if (commandEnv == null) {
            commandEnv = getEnvironmentDescriptor(cmdDesc.getCommand());
        }
        return DEFAULT_EXECUTOR_INSTANCE.exec(cmdDesc, params,
                EnvironmentDescriptor.newInstance(globalEnv, commandEnv));
    }
